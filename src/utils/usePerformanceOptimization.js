...
/**
 * Wraps a handler and logs execution time in dev. Not a Hook — safe to use anywhere.
 * For memoized handlers inside components, wrap the result in `useCallback` yourself.
 */
export const createOptimizedHandler = (handler, handlerName = 'handler') => {
  return (...args) => {
    const startTime = performance.now();
